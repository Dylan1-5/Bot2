...
export const handleKick = async (conn, from, msg, args, participants, groupMetadata, usedPrefix, command) => {
    ...
    // 3. SACAR POR @ETIQUETA, CITA DE MENSAJE O NÚMERO DIRECTO
    ...
    if (mentionedJid.length > 0) {
        targetRaw = mentionedJid[0] // Si usó @Mención
    } else if (quotedMsgParticipant) {
        targetRaw = quotedMsgParticipant // Si respondió al mensaje de alguien
    } else if (args[0] && args[0].replace(/\D/g, '').length > 5) {
        targetRaw = args[0].replace(/\D/g, '') + '@s.whatsapp.net' // Si escribió el número directo
    }

    // Si no se proporcionó a quién sacar
    if (!targetRaw) {
        return reply(`《✧》 Por favor, Etiqueta (@usuario) o responde al *mensaje* de la *persona* que quieres eliminar.\n\n✎ *Opciones especiales:*\n> *${usedPrefix || '/'}${command} num +57* - Eliminar todos los usuarios con prefijo +57\n> *${usedPrefix || '/'}${command} listnum +56* - Listar usuarios con prefijo +56\n> *${usedPrefix || '/'}${command} all* - Eliminar todos los usuarios`)
    }
