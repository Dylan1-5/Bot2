...
const STICKER_BATCH_MAX = 100
const STICKER_BATCH_GAP = 60 * 1000
const stickerMediaQueue = new Map()
const stickerProcessingChats = new Set()
const logger = P({ level: 'silent' })

if (!fs.existsSync(TMP_DIR)) fs.mkdirSync(TMP_DIR, { recursive: true })

const getAlbumKey = msg => {
    const raw = msg?.message || {}
    const type = Object.keys(raw)[0] || ''
    const media = raw[type] || {}
    const context = media.contextInfo || raw.messageContextInfo || {}
    const association = context.messageAssociation
    return association?.parentMessageKey?.id || association?.parentMessageKey?.remoteJid || ''
}

export const rememberStickerMedia = (msg, senderNumber = '') => {
    const chat = msg?.key?.remoteJid
    if (chat && stickerProcessingChats.has(chat)) return
    const rawMessage = msg?.message || {}
    const type = Object.keys(rawMessage)[0] || ''
    // También detecta imágenes envueltas por mensajes de álbum, ephemeral o view-once.
    if (!/imageMessage|videoMessage/.test(JSON.stringify(rawMessage))) return

    if (!chat) return

    const current = stickerMediaQueue.get(chat) || []
    const messageId = msg?.key?.id
    if (messageId && current.some(item => item.msg?.key?.id === messageId)) return

    const now = Date.now()
    const previous = current[current.length - 1]
    const albumKey = getAlbumKey(msg)
    const batchId = albumKey
        || (previous && now - previous.createdAt <= STICKER_BATCH_GAP ? previous.batchId : `${now}`)

    current.push({ msg, senderNumber, createdAt: now, batchId })
...
const getQuotedMessageId = msg => getContextInfo(msg)?.stanzaId || ''
...
export const handleSticker = async (conn, msg, options = {}) => {
    ...
    try {
        ...
        if (options.all === true || args.some(arg => String(arg).trim().toLowerCase() === 'all')) {
            ...
            const allQueued = (stickerMediaQueue.get(chat) || [])
                .sort((a, b) => a.createdAt - b.createdAt)
            const quotedId = getQuotedMessageId(msg)
            const anchor = quotedId
                ? allQueued.find(item => item.msg?.key?.id === quotedId)
                : undefined

            if (!anchor) {
                return reply('《✧》 Responde a una imagen del grupo y usa *sticker all* para convertir únicamente ese álbum.')
            }

            const albumBatch = allQueued.filter(item => item.batchId === anchor.batchId)
            const ownQueued = albumBatch.filter(item => !item.senderNumber || item.senderNumber === requesterNumber)
            // El mensaje respondido funciona como ancla: solo se usa su álbum/lote.
            const queued = ownQueued.length > 0 ? ownQueued : albumBatch

            if (queued.length === 0) {
                return reply('《✧》 No encontré las imágenes del álbum respondido. Vuelve a responder una de ellas.')
            }

            stickerMediaQueue.set(chat, [])
            const singleArgs = args.filter(arg => String(arg).toLowerCase() !== 'all')
            stickerProcessingChats.add(chat)
            ...
            try {
                ...
                stickerProcessingChats.delete(chat)
            }
            return
        }
