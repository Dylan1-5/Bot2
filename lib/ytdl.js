...
async function resolveYtDlp() {
    ...
    for (const candidate of directCandidates) {
        ...
        // En Termux puede funcionar por ruta absoluta
        // aunque el proceso no herede PATH.
        ...
        ytDlpRunner = (args) =>
            execFilePromise(candidate, args, {
                timeout: 180000,
    ...
    // Permite usar yt-dlp instalado como módulo de Python.
...
async function resolveFfmpeg() {
    ...
    for (const candidate of candidates) {
        ...
        // No depender únicamente del PATH.
        if (candidate.includes(path.sep)) {
            if (!isFile(candidate)) continue
        } else if (!(await canRun(candidate))) {
            continue
...
async function runYtDlpWithFallback(
    ...
    for (const client of CLIENT_COMBOS) {
        ...
        try {
            ...
            console.warn(
                `[yt-dlp] Cliente ${client} falló: ${error.message}`
            )
        }
    }
    ...
    throw (
        lastError ||
        new Error(
            'No se pudo descargar el enlace con ningún cliente de YouTube.'
        )
    )
}
...
export async function downloadMedia(
    ...
    const commonArgs = [
        ...
        // Más fragmentos = mayor velocidad.
    ...
    try {
        ...
        if (
            ...
            // WhatsApp necesita un contenedor OGG válido.
            const finalPath =
                path.join(
                    tmpDir,
                    `${timestamp}_${isMp3 ? 'mp3' : 'vn'}.${isMp3 ? 'mp3' : 'ogg'}`
                )
            ...
            if (
                ...
                fs.statSync(finalPath).size < 1000
            ) {
                throw new Error(
                    'FFmpeg generó un archivo de audio vacío o inválido.'
                )
            }
        ...
        if (
            ...
            throw new Error(
                'Esta canción tiene protección DRM y no se puede descargar.'
            )
        }
...
// Busca varios resultados y descarga
// el primero que funcione.
export async function downloadFromSearch(
    ...
    if (
        ...
        throw new Error(
            'No se recibió una búsqueda válida.'
        )
    }
    ...
    for (
        ...
        for (
            ...
            try {
                ...
                console.warn(
                    `[yt-dlp] Se omite "${video.title}": ${error.message}`
                )
            }
        }
    }
    ...
    throw (
        lastError ||
        new Error(
            'No se pudo descargar ningún resultado de YouTube.'
        )
    )
}
