...
export async function downloadMedia(url, format = 'vn') {
    ...
    try {
        ...
        if (format === 'vn' || format === 'mp3') {
            ...
            await runYtDlp([
                ...commonArgs,
                // Algunos clientes de YouTube no exponen webm/m4a; usar cualquier audio disponible evita "Requested format is not available".
                '-f', 'bestaudio/best',
                '-o', rawTemplate,
                url
            ])
        ...
        await runYtDlp([
            ...commonArgs,
            // No limitar extensiones: la disponibilidad cambia según el cliente de YouTube.
