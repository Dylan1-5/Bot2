...
export async function downloadMedia(url, format = 'vn') {
    ...
    // Banderas optimizadas para m谩xima velocidad de descarga y menor uso de CPU
    const speedFlags = [
        `--user-agent "${randomUA}"`,
        ...
        '--no-cache-dir',
        '--socket-timeout 10',
        '--concurrent-fragments 4', // Descarga hasta 4 fragmentos simult谩neamente
        '--buffer-size 16k'
    ].join(' ')
    ...
    if (format === 'vn' || format === 'mp3') {
        // Priorizar formatos Opus/WebM ligeros de YouTube para reducir trabajo de conversi贸n
        const downloadCmd = `yt-dlp ${speedFlags} -f "bestaudio[ext=webm]/bestaudio[ext=m4a]/bestaudio/best" -o "${rawTemplate}" --no-playlist "${url}"`
        ...
        try {
            ...
            // Conversi贸n ultrarr谩pida con hilos optimizados para procesadores ARM de m贸viles
            ...
            if (error.message.includes('DRM protected')) {
                throw new Error('Esta canci贸n tiene protecci贸n DRM y no se puede descargar.')
            }
            throw new Error(`Error en descarga r谩pida: ${error.message}`)
        }
    } else {
        const videoPath = path.join(tmpDir, `${timestamp}_video.mp4`)
        const videoCmd = `yt-dlp ${speedFlags} -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best" --merge-output-format mp4 -o "${videoPath}" --no-playlist "${url}"`
